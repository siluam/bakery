...
If the final output is an iterable, return it wrapped in an [[https://github.com/rsalmei/alive-progress][alive-progress]] progress bar:

#+begin_src hy
<<m/(setting="progress", value="False")>>
#+end_src
...
If ~__str__~ doesn't exist, ~__repr__~ called by ~(inspect self)~ trigger in infinite loop when getting the title of the report, since it's not being passed in.
